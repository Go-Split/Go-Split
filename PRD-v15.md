...
> **版本**：v0.15（L25：核心資料模型補正——rest／detail.id／手動分攤與自訂金額／event 基本欄位／命名統一）
> **範圍**：MVP / R1
...
|------|----------|-------------|--------|
| **MVP / R1** | • 條件式分攤引擎（F1-F3，Approach A 含零權重排除）<br>• 手動記帳（G，收據見 C24）<br>• 結算+債務極小化（H1）<br>• 付款流向清單（H3，純檢視）<br>• 三角色權限、群組管理（D/E）<br>• 邀請碼加入（A/C）<br>• 情境模板（K：自訂＋烤肉/露營）| • Approach B 指定群組百分比<br>• OCR、自動分類<br>• 真實金流串接<br>• 催收提醒<br>• 4 個未規劃模板內容 | 引擎:高<br>其餘:中 |
...
| **L10** 🔴 | 付款流向用了**兩套演算法** | H1/H2/H3/J | §5.2 定案「債務極小化（貪婪法）」 | `computeCtx()` **同時算了兩套**：`transfers`（貪婪極小化，app.js:391）與 `flowRows`（**以主辦為中心的 hub 星狀**，app.js:404，spec 從未定義）。且畫面各用各的——**settle／settledEvent／archived 用 hub；payments 用貪婪** | **同活動兩畫面會顯示不同的轉帳對象**，直擊「可驗證信任」USP。<br>**策略決策擱置中**（業務情境傾向 hub：有固定主辦、大家轉給主辦）。<br>**但收斂不可擱置**：先合併為單一出口 `computeTransfers(nets, {strategy})`，四個畫面全投影自同一份輸出；策略日後改參數即可，並存進結帳快照避免數字漂移 |
...
**對應規格**：§5.1 H0、§5.2 H1（含債務極小化）、§5.3 H2、§5.4 H3、§6.3 邀請碼失效、C29（編輯入口隱藏）、**C31（繳款標記移除）**
...
*文件結束。本 spec 為 v0.15，A→K 十一區已鎖定核心規格。*
...
| §13.2 落差待修 | **L4** tags[] 收斂單一 itemTag、**L5** Google OAuth + session 架構、**L7/L18** 收據文案（兩處）、**L8** 擋存防呆接到整筆提交、**L10** 付款流向雙軌收斂、**L12** 標籤擋刪、**L14** 分攤可編輯性鎖定、**L24** 繳款標記移除（C31）、**L15** settled 每活動持久化、**L16** archive guard、**L17** 邀請碼真產生、**L19** 權限收斂、**L20** 前端草稿層、**L21-b** 規則刪除防呆、**L22** 除零擋存 |
