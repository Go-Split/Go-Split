# SPEC-ENGINE — 分攤引擎規格 v1.1

> **對應 PRD**：群體活動分帳工具 PRD v0.16 §3（F 區）、§5.2（H1）
> **v1.1 變更**：L10 定案——`computeTransfers` 改為**主辦中心（hub）單一策略**，移除 `greedy`；§6 AC 全部重寫；§10 移除 L10 待定項。
> **套件**：`packages/engine`
...
│  ├─ computeTransfers.ts   # SPEC-H1（§6）hub only
│  └─ index.ts
└─ test/
...
function computeTransfers(
  ...
  hubId: string                                // 主辦人 memberId
): { from: string; to: string; amount: number }[]
```

> **v1.1 變更**：L10 已定案採**主辦中心（hub）**，`strategy` 參數移除，不實作貪婪法（R1 不會呼叫，保留即為死碼）。策略記錄改由**結帳快照**負責（`strategy: 'hub'` + `hubId`），用於日後策略若變更時解釋歷史數字。

### 6.2 業務規則（PRD §5.2）

> 所有應付款項**統一交給主辦人**；若協辦者代墊較多，由**主辦人轉給協辦者**；
> **參與者與協辦者之間不產生任何金額流向**。
...
```
對「主辦以外」的每位成員：
  net < 0 → push { from: memberId, to: hubId,    amount: -net }
  net > 0 → push { from: hubId,    to: memberId, amount:  net }
  net = 0 → 不產生

主辦本身：不產生任何「付給自己」的列
```

**主辦自身淨額無須特別處理**——因 Σnet = 0，主辦收到與付出的差額恰為其自身淨額：
...
prototype 目前**同時算了兩套**且畫面各用各的：`transfers`（貪婪極小化，app.js:391）給 payments 頁；`flowRows`（hub 星狀，app.js:404）給 settle／settledEvent／archived 三頁。**同一活動兩個畫面會顯示不同的轉帳對象**（PRD L10）。

L10 定案後：**移除貪婪法**，四個畫面全部投影自本函式的單一輸出。

### 6.5 AC

| # | Given | When | Then |
|---|-------|------|------|
| H1-01 | 主辦 +4000、乙 −3000、丙 −1000 | computeTransfers | 乙→主辦 3000、丙→主辦 1000（2 筆） |
| H1-02 | **主辦 0、阿豪 +3000、小美 −1500、佳蓉 −1500** | computeTransfers | 小美→主辦 1500、佳蓉→主辦 1500、**主辦→阿豪 3000**（3 筆）<br>⭐ **小美／佳蓉與阿豪之間無任何轉帳** —— L10 核心案例 |
| H1-03 | 主辦 −800、阿豪 +800 | computeTransfers | 主辦→阿豪 800（1 筆）；**無「主辦→主辦」的列** |
| H1-04 | 全員 net = 0 | computeTransfers | 回 [] |
| H1-05 | 6 人、其中 4 人 net≠0（含主辦） | computeTransfers | 筆數 = **3**（主辦以外 net≠0 的人數） |
| H1-06 | 主辦 net≠0、其餘全為 0 | computeTransfers | 回 []（Σnet=0 的前提下此情境不可能發生，應由不變量三攔截） |
| H1-07 | 任意 nets | computeTransfers | **Σ(流入主辦) − Σ(流出主辦) === −主辦.net**（自動平衡驗證） |
| H1-08 | `nets` 中找不到 `hubId` | computeTransfers | 回報錯誤——每活動恰好一位主辦，此為呼叫端錯誤 |

### 6.6 不做的事

| 項目 | 說明 |
|------|------|
| 最少轉帳筆數 | **不追求**。hub 模型下筆數固定為「主辦以外 net≠0 的人數」，不做最佳化 |
| 主辦代收代付的額外提示 | 主辦淨額為 0 仍可能需代收代付（H1-02）。**H3 照實列出三筆即可**，引擎不產生任何額外標記（PRD §5.2 已確認） |
| 策略切換 | R1 不實作。`strategy` 由快照記錄，非引擎參數 |
...
| 5 | `computeTransfers` + H1 AC | L10 收斂完成（hub only） |
